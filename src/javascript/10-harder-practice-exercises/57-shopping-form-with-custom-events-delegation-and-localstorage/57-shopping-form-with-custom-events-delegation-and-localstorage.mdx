...
![building shopping list](./806.png)
...
![learn about emitting custom events](./807.png)
...
![install parcel bundler globally on machine](./810.png)
...
![start the server](./811.png)

![shopping list ui in browser](./812.png)
...
![search values on typing in input box](./813.png)
...
![finished shopping list example](./814.png)
...
![type eggs in input and click add item](./815.png)

![?item=egg gets appended in url](./816.png)
...
![submitted!! in console log](./817.png)
...
![console log e.currentTarget](./818.png)
...
![store the form as global variable](./819.png)
...
![console.dir(temp1) in console tab](./820.png)
...
![modify the log of e.currentTarget to e.currentTarget.item](./821.png)
...
![captured the value of eggs in console](./822.png)
...
![count logs increasing by adding more items](./824.png)
...
![calling map method on the names array](./825.png)
...
![looping over the displayItems](./826.png)
...
![join method on html variable returns one string](./827.png)
...
![typing an item and submit will be displayed](./828.png)
...
![shopping list view after adding some items](./829.png)
...
![firing off event from the ul list](./830.png)
...
![custom event log in console](./831.png)
...
![running the custom event using new CustomEvent('PIZZAS HERE')](./833.png)
...
![shopping list with elements in console](./834.png)
...
![items stored in localStorage](./835.png)
...
![logging localStorage in console](./836.png)
...
![clearning the localStorage if something wrong happens](./837.png)
...
![localStorage storing values as object Object](./838.png)
...
![items in localStorage stored accurately](./841.png)
...
![number of buttons returns a nodelist of 5 on querying with querySelectorAll](./842.png)

However, if you move that code further up, above the event listener declarations, we get none.

![moving the code above the event listener](./843.png)
...
![clicking the delete button](./847.png)
...
![html button id in the element tab](./848.png)
...
![deleteItem function logging the id in console](./849.png)
...
![pressing delete button next to item returns empty array](./850.png)
...
![on clicking the button, you need the item to be removed](./851.png)
...
![on cliking the delete button for an item, we are getting return with the remaining items](./852.png)
...
![mark as complete in console](./853.png)
...
![usage of not operator in console](./854.png)
...
![LocalStorage getting updated along with all the events](./855.png)
